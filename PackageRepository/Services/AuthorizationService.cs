...
namespace PackageRepository.Services {
    public interface IAuthorizationService {
        Task<bool> IsAuthorizedAsync(UserContext context, long thingId, Permissions requiredPermissions);
    }

    public class AuthorizationService : IAuthorizationService {
        private readonly IThingRepository _thingRepository;

        public AuthorizationService(IThingRepository thingRepository) {
            _thingRepository = thingRepository;
        }

        public async Task<bool> IsAuthorizedAsync(UserContext context, long thingId, Permissions requiredPermissions) {
            var thingPermissions = await _thingRepository.GetThingPermissionsAsync(thingId).ConfigureAwait(false);
